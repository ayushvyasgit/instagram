...
export const postAPI = {
  ...
  // ── Existing offset-based methods (UNCHANGED) ──────────────────────────────
  ...
  // ── NEW: cursor-based methods (added alongside, not replacing) ─────────────

  /**
   * Fetch feed page using cursor pagination.
   *
   * @param limit      - page size (default 10)
   * @param cursor     - opaque cursor string from previous response (null = first page)
   * @param direction  - "next" (older posts) | "prev" (newer posts)
   *
   * Response shape:
   *   { posts, pagination: { nextCursor, prevCursor, hasNextPage, hasPreviousPage, limit } }
   */
  getFeedCursor: (limit = 10, cursor: string | null = null, direction: 'next' | 'prev' = 'next') =>
    api.get('/posts', {
      params: {
        limit,
  ...
  /**
   * Fetch a user's posts using cursor pagination.
   *
   * @param userId
   * @param limit
   * @param cursor     - opaque cursor string (null = first page)

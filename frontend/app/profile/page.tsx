...
import { setUserPosts, removePost } from '@/src/store/postsSlice';
...
export default function ProfilePage() {
  ...
  const loadMyPosts = async () => {
    ...
    try {
      setLoading(true);
      const response = await postAPI.getUserPosts(user.id, 1, 50);
      dispatch(setUserPosts(response.data.data.posts));
    } catch (error) {

...
export default function FeedPage() {
  ...
  const {
    ...
    feedTotal,
    feedLoading,
    // NEW: cursor state read from slice
    feedNextCursor,
    feedPrevCursor,
    feedHasNextPage,
    feedHasPreviousPage,
  ...
  // ── Existing offset-based loader (UNCHANGED, still used by loadInitialFeed) ──
  const loadFeedPage = async (page: number) => {
    try {
      ...
      dispatch(setFeedPosts({ posts, total: pagination.total, hasMore: pagination.hasMore, page }));
    } catch { dispatch(setFeedLoading(false)); }
  };

  // ── NEW: cursor-based page loader ─────────────────────────────────────────
  /**
   * Load a feed page using cursor pagination.
   * Dispatches setFeedPostsCursor which stores cursor state in the slice
   * alongside (not replacing) the existing offset state.
   *
   * @param cursor    - opaque cursor string from slice, or null for first page
   * @param direction - "next" (older) | "prev" (newer)
   */
  const loadFeedCursor = async (cursor: string | null, direction: 'next' | 'prev') => {
    try {
      dispatch(setFeedLoading(true));
      const res = await postAPI.getFeedCursor(10, cursor, direction);
      const { posts, pagination } = res.data.data;
      dispatch(
        setFeedPostsCursor({
          posts,
  ...
  // ── NEW: cursor navigation handlers ───────────────────────────────────────
  const handleCursorNext = () => {
    if (feedHasNextPage && feedNextCursor) {
      loadFeedCursor(feedNextCursor, 'next');
    }
  };

  const handleCursorPrev = () => {
    if (feedHasPreviousPage && feedPrevCursor) {
      loadFeedCursor(feedPrevCursor, 'prev');
    }
  };
  ...
  // loadInitialFeed now kicks off cursor-based first page load
  // (falls back to offset if cursor API isn't ready yet)
  const loadInitialFeed = async () => {
    loadFeedCursor(null, 'next');
  };
      ...
      <div className="feed-root">
        ...
        <main className="feed-main">
          <div className="feed-col">
            ...
            {/* ── Pagination ── */}
            {/*
              Uses cursor state when available (feedNextCursor/feedPrevCursor),
              falls back to offset state (feedPage/feedTotal/feedHasMore).
              Both branches share the same button UI.
            */}
            {feedPosts.length > 0 && (
              <div className="feed-pagination">
                <button
                  className="pagination-btn"
                  onClick={feedPrevCursor !== null ? handleCursorPrev : handlePrevPage}
                  disabled={
                    feedPrevCursor !== null
                      ? !feedHasPreviousPage   // cursor mode
                      : feedPage === 1         // offset fallback
                  }
                >
                  Previous
                </button>
                ...
                <span className="pagination-info">
                  {/* Show page number in offset mode; hide in cursor mode since we have no total */}
                  {feedPrevCursor === null && feedNextCursor === null
                    ? `Page ${feedPage} of ${Math.ceil(feedTotal / 10) || 1}`
                    : 'Browse posts'}
                </span>

                <button
                  className="pagination-btn"
                  onClick={feedNextCursor !== null ? handleCursorNext : handleNextPage}
                  disabled={
                    feedNextCursor !== null
                      ? !feedHasNextPage        // cursor mode
                      : !feedHasMore            // offset fallback
                  }
                >
                  Next
                </button>
